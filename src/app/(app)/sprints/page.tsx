...
const RETRO = [
  { k: "went-well", title: "What went well", color: "var(--good)", items: ["Velocity hit 41/42 pts — best sprint this quarter", "Auto status updates saved ~4h of ticket admin", "Zero carry-over bugs from Sprint 12"] },
  ...
  { k: "actions", title: "AI-suggested actions", color: "var(--brand)", items: ["Add a daily 30-min design review slot (owner: Priya)", "Apply 1.25x multiplier to data-pipeline estimates", "Require AI-generated acceptance criteria before sprint entry"] },
];
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      <div className="float-up flex flex-wrap items-center justify-between gap-3">
        ...
        <button className="btn-primary px-4 py-2 text-sm"><Sparkles size={14} /> Auto-plan Sprint 15</button>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        ...
        <Card className="float-up"><Stat label="Success prediction" value="81%" sub="AI: recoverable if PORT-91 clears today" subColor="var(--warn)" /></Card>
        <Card className="float-up"><Stat label="Capacity used" value="86%" sub="Healthy buffer · Rohan rebalanced" subColor="var(--good)" /></Card>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        ...
        <Card title="Sprint 15 — AI Draft" action={<Sparkles size={14} style={{ color: "var(--brand)" }} />}>
          <div className="rounded-xl p-3.5 text-[13px] leading-relaxed" style={{ background: "var(--brand-soft)" }}>
            <p className="font-semibold" style={{ color: "var(--brand)" }}>Goal: Ship KYC + clear design debt</p>
            <ul className="mt-2 space-y-1.5 text-ink-2">
              <li>· 12 items, 34 pts (velocity avg: 35)</li>
              <li>· Rohan capped at 82% capacity</li>
              <li>· BANK-151 sequenced after blocker</li>
              <li>· 2 buffer slots for review findings</li>
            </ul>
          </div>
          <div className="mt-3 flex gap-2">
            <button className="btn-primary flex-1 px-3 py-2 text-xs">Apply plan</button>
            <button className="btn-ghost flex-1 px-3 py-2 text-xs">Edit draft</button>
          </div>
        </Card>
      </div>
