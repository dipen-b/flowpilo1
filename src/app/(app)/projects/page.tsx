...
import { projects, memberById, riskMeta, tasksForProject } from "@/lib/data";
    ...
    <div className="mx-auto max-w-7xl space-y-5">
      <div className="float-up flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-2">4 active · AI is watching all of them</p>
        </div>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {projects.map((p, i) => {
          const open = tasksForProject(p.id).filter((t) => t.status !== "done").length;
          return (
            <Link key={p.id} href={`/projects/${p.id}`} className="float-up" style={{ animationDelay: `${i * 60}ms` }}>
              <Card className="h-full transition hover:-translate-y-0.5 hover:shadow-lg">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-ink-3">{p.key} · {p.sprint} · due {p.dueDate}</p>
                    </div>
                  </div>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <AvatarStack people={p.members.map(memberById)} />
                  <div className="text-right text-xs text-ink-3">
                    <p className="tabular">{open} open tasks</p>
