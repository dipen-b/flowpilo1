...
async function main() {
  ...
  const projectSeed = [
    ...
    { id: "p4", name: "AI Support Chatbot", key: "CHAT", emoji: "🤖", health: 91, risk: "good", progress: 83, dueDate: "Jul 25", lead: "u4", forecastDate: "Jul 24", forecastConfidence: 94, forecastDelta: "1 day early", summary: "Ahead of plan. Intent coverage at 96%; release candidate ready Friday." },
  ];
  ...
  const taskSeed = [
    ...
    { key: "BANK-138", title: "Transaction history infinite scroll + filters", project: "p1", assignee: "u4", priority: "high", status: "in_review", labels: "frontend", due: "Jul 11", estimate: 8, spent: 8, aiFlag: "PR approved — AI suggests moving to Done", sprint: true },
    { key: "BANK-151", title: "KYC document upload with OCR validation", project: "p1", assignee: "u3", priority: "high", status: "todo", labels: "backend,compliance", due: "Jul 16", estimate: 16, spent: 0, aiFlag: "Dependency: blocked by BANK-149", sprint: true },
    ...
    { key: "PORT-91", title: "Design review: notification preferences UX", project: "p2", assignee: "u2", priority: "urgent", status: "todo", labels: "design", due: "Jul 10", estimate: 4, spent: 0, aiFlag: "Overdue risk — blocking 5 tasks" },
    { key: "DATA-45", title: "Migrate events pipeline to new warehouse schema", project: "p3", assignee: "u3", priority: "urgent", status: "in_progress", labels: "data,infra", due: "Jul 09", estimate: 24, spent: 31, aiFlag: "Overdue — AI recommends splitting into 3 subtasks" },
    { key: "CHAT-29", title: "Fine-tune fallback responses for billing intents", project: "p4", assignee: "u5", priority: "medium", status: "in_review", labels: "ai", due: "Jul 15", estimate: 6, spent: 5 },
    ...
    { key: "CHAT-31", title: "Human handoff flow when confidence < 70%", project: "p4", assignee: "u6", priority: "high", status: "todo", labels: "ai,ux", due: "Jul 17", estimate: 8, spent: 0 },
  ...
  await db.aiInsight.createMany({
    data: [
      { level: "critical", kind: "risk", title: "Data Platform Migration will miss Jul 18 deadline", detail: "Velocity trend predicts Aug 04 delivery (41% confidence). Rohan is at 128% capacity.", action: "Rebalance 2 tasks to Aarav", projectId: projects.p3 },
      { level: "warning", kind: "risk", title: "Design review bottleneck in Customer Portal", detail: "PORT-91 is overdue and blocks 5 downstream frontend tasks.", action: "Escalate review to today", projectId: projects.p2 },
      { level: "warning", kind: "risk", title: "Rohan Patel shows burnout signals", detail: "51h assigned vs 40h capacity, 3 weeks running. Productivity down 14%.", action: "Reduce load by 11h", projectId: projects.p3 },
      { level: "good", kind: "recommendation", title: "Move BANK-138 to Done", detail: "Its pull request was approved and merged 2h ago.", action: "Apply", projectId: projects.p1 },
      { level: "good", kind: "recommendation", title: "Split DATA-45 into 3 subtasks", detail: "It's 29% over estimate and still growing.", action: "Split with AI", projectId: projects.p3 },
    ],
  });
  ...
  await db.activityLog.createMany({
    data: [
      ...
      { who: "FlowPilot AI", what: "auto-updated BANK-135 to Done from commit activity" },
      { who: "Priya Mehta", what: "commented on PORT-88: “Use the new tokens for spacing”" },
      { who: "Dev Kapoor", what: "logged 3h on CHAT-29" },
      { who: "FlowPilot AI", what: "generated Sprint 13 retrospective summary" },
